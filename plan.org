...
***** [@kostya] test the threading model

Do we need to call ~m0_thread_adopt()~ from Python thread?

***** [@mandar]

**** HOLD C (libcurl, libjson)                                      :HOLD:

**** HOLD Go (Consul Go bindings)                                   :HOLD:

- Learning curve, however short, is still to be accounted for.
- Also we may have hard time selling the idea of non-mainstream language implementation to management.
...
**** write an RFC for "initial-rg.json"
...
Handle failures of m0d processes.  Consul watch to trigger Pacemaker
to do failover?
